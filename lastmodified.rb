...
=> "seconds_to_days requires Numeric."
...
def seconds_to_days(seconds)
  return "seconds_to_days requires Numeric." unless seconds.is_a?(Numeric)
...
def last_modified(file_name = nil, fake_current_time_for_testing = false)
  ...
  if File.exists?(file_name) && File.file?(file_name)
    ...
    puts "#{file_name} is not a valid file."
  end
end